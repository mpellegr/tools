/*
ColorControlCentralizer_v02_06.jsx

Creates a floating UI to centralize project colors into a COLOR_CONTROL comp.
Analysis mode can be PRECISO, with exact matching, or LIMPIO, with a small
RGB tolerance for visually similar colors. Previous runs are detected through a
rare text identifier and existing control effects; existing controls are reused.
...
(function colorControlCentralizerUI(thisObj) {
    ...
    var PALETTE_LAYER_NAME = "COLOR_CONTROL_PALETTE";
    var EFFECT_PREFIX = "Color_";
    var IDENTIFIER_PREFIX = "MML_COLOR_CONTROL_CENTRALIZER::";
    var IDENTIFIER_VALUE = IDENTIFIER_PREFIX + "v02_06";
    var APPROXIMATE_TOLERANCE = 2;
    var SWATCH_SIZE = 50;
    var SWATCH_GAP = 12;
    ...
    function runCentralizer(tolerance, analysisModeLabel, onReviewRowsReady) {
        if (!app.project) {
        ...
        function isIdentifierLayer(layer) {
            if (!layer || layer.name !== CONTROL_LAYER_NAME) {
                return false;
            }
            ...
            try {
                ...
                return textDocument.text.indexOf(IDENTIFIER_PREFIX) === 0;
            } catch (err) {
                return false;
            }
        }

        function layerHasColorControlEffects(layer) {
            try {
                ...
                for (var i = 1; i <= effects.numProperties; i++) {
                    ...
                    if (effect && effect.matchName === "ADBE Color Control") {
                        return true;
                    }
                }
            } catch (err) {
        ...
        function detectPreviousRun(controlComp) {
            if (!controlComp) {
                return false;
            }

            for (var i = 1; i <= controlComp.numLayers; i++) {
                var layer = controlComp.layer(i);
                if (isIdentifierLayer(layer) || layerHasColorControlEffects(layer)) {
                    return true;
                }
            }
        ...
        function findNamedLayer(comp, layerName) {
        ...
        function createIdentifierTextLayer(controlComp) {
            var layer = controlComp.layers.addText(IDENTIFIER_VALUE);
        ...
        function getOrCreateControlLayer(controlComp) {
            var existing = findNamedLayer(controlComp, CONTROL_LAYER_NAME);
            if (existing && (isIdentifierLayer(existing) || layerHasColorControlEffects(existing))) {
                return existing;
            }

            return createIdentifierTextLayer(controlComp);
        }

        function getOrCreatePaletteLayer(controlComp) {
            var existing = findNamedLayer(controlComp, PALETTE_LAYER_NAME);
            if (existing) {
                existing.remove();
            }

            var layer = controlComp.layers.addShape();
            layer.name = PALETTE_LAYER_NAME;
            layer.property("ADBE Transform Group").property("ADBE Position").setValue([0, 0]);
            layer.property("ADBE Transform Group").property("ADBE Anchor Point").setValue([0, 0]);
            return layer;
        }

        function loadExistingColorControls(controlLayer) {
            ...
            for (var i = 1; i <= effects.numProperties; i++) {
                ...
                try {
                    var color = normalizeColor(effect.property(1).value);
                    uniqueColors.push({
                        key: colorKeyFromRgb(colorToRgb255(color)),
                        rgb: colorToRgb255(color),
                        color: color,
        ...
        function writeCsvLog() {
            ...
            var file = new File(getScriptFolder().fsName + "/ColorControlCentralizer_v02_06_log_" + getTimestamp() + ".csv");
            file.encoding = "UTF-8";
            ...
            file.write("\uFEFF");
            file.writeln("Script Version," + csvEscape("v02_06"));
            file.writeln("Timestamp," + csvEscape(getTimestamp()));
            file.writeln("Analysis Mode," + csvEscape(analysisModeLabel));
            file.writeln("Previous Run Detected," + csvEscape(previousRunDetected ? "YES" : "NO"));
            file.writeln("");
        ...
        function createPaletteSwatches(controlLayer) {
            ...
            for (var i = 0; i < uniqueColors.length; i++) {
                ...
                fill.property("ADBE Vector Fill Color").expression = 'comp("' + CONTROL_COMP_NAME + '").layer("' + CONTROL_LAYER_NAME + '").effect("' + info.effectName + '")(1)';
        ...
        try {
            ...
            previousRunDetected = detectPreviousRun(controlComp);
            if (previousRunDetected) {
                alert("Se detecto una ejecucion previa. No se van a sobrescribir los controles existentes pero se intentara completar colores/casos adicionales.");
            }

            var controlLayer = getOrCreateControlLayer(controlComp);
            var paletteLayer = getOrCreatePaletteLayer(controlComp);
            loadExistingColorControls(controlLayer);

            scanProjectColors(controlComp);
            scanTextBaseColors(controlComp);
            createColorControls(controlLayer);
            createPaletteSwatches(paletteLayer);
            ...
            alert(
                ...
                "Analisis: " + analysisModeLabel + "\n" +
                "Ejecucion previa detectada: " + (previousRunDetected ? "SI" : "NO") + "\n" +
    ...
    function buildUI(thisObjRef) {
        var win = (thisObjRef instanceof Panel)
            ? thisObjRef
            : new Window("palette", "Color Control Centralizer v02_06", undefined, { resizeable: true });
        ...
        var title = win.add("statictext", undefined, "Color Control Centralizer v02_06");
        title.alignment = ["fill", "top"];

        var analysisGroup = win.add("group");
        analysisGroup.orientation = "row";
        analysisGroup.alignChildren = ["center", "center"];
        analysisGroup.add("statictext", undefined, "Analisis");

        var preciseMode = analysisGroup.add("radiobutton", undefined, "PRECISO");
        var cleanMode = analysisGroup.add("radiobutton", undefined, "LIMPIO");
        ...
        function getSortValue(row, sortName) {
            if (sortName === "Comp") {
                return row.compName;
            }
            if (sortName === "Layer") {
            ...
            if (sortName === "Property") {
                return row.propertyPath;
            }
            if (sortName === "Reason") {
                return row.reason;
            }
            if (sortName === "Expression") {
        ...
        function sortReviewRows(rows) {
            var sortName = sortDropdown.selection ? sortDropdown.selection.text : "Comp";
            rows.sort(function(a, b) {
                var valueA = String(getSortValue(a, sortName)).toLowerCase();
                var valueB = String(getSortValue(b, sortName)).toLowerCase();
        ...
        function renderReviewList() {
            clearReviewList();
            reviewLabel.text = "Omitted / manual review: " + currentReviewRows.length;
            selectedExpression.text = "";
            sortReviewRows(currentReviewRows);

            for (var i = 0; i < currentReviewRows.length; i++) {
                var row = currentReviewRows[i];
        ...
        function getPrimarySelection() {
            if (!reviewList.selection) {
                return null;
            }

            if (reviewList.selection instanceof Array) {
                return reviewList.selection.length > 0 ? reviewList.selection[0] : null;
            }

            return reviewList.selection;
        }

        function updateSelectedExpression() {
            var selectedItem = getPrimarySelection();
            if (selectedItem && selectedItem.rowData) {
                selectedExpression.text = selectedItem.rowData.expression || "";
            } else {
                selectedExpression.text = "";
            }
        }
        ...
        reviewList.onDoubleClick = function() {
            var selectedItem = getPrimarySelection();
            if (selectedItem && selectedItem.rowData) {
                jumpToReviewRow(selectedItem.rowData);
            }
        };
        ...
        applyExpressionButton.onClick = function() {
            ...
            try {
                ...
                selectedItem.subItems[2].text = row.reason;
        ...
        analyzeButton.onClick = function() {
            ...
            runCentralizer(analysisSettings.tolerance, analysisSettings.label, populateReviewList);
        };
